...
func (api *taskAPI) MountTo(r fiber.Router) {
	r.
		// middlewares
		Use(ValidateHeadersMiddleware(mime.ApplicationMPEG_NBMP_TDD_JSON)).
		// API
...
func (api *taskAPI) handleRequest(svcCall func(context.Context, *nbmpv2.Task) error) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// note: the NBMP standard requires (SHOULD) a link object in the TDD response. The JSON schema definition does not
